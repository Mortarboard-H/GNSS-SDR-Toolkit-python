...
## ----------------------------------------------------
# created by mortarboard in 2022 06 01
...
class Trackor(Tracking):
    def __init__(self,
        ...
        self.phase_loopfilter=EulerOrder2Direct(0.5,1,self.accu_time/1e3)
        self.code_loopfilter=EulerOrder2Direct(0.5,1,self.accu_time/1e3)
        pass
...
# This method revert the nagetive results to positives and later stores them in the container
class TrackPerMs(Tracking):
    def __init__(self,
        ...
        self.phase_loopfilter=EulerOrder2Direct(0.1,1,self._accu_time_pre_round/1e3)
        # define the loop filter of code phase to be used
        self.code_loopfilter=EulerOrder2Direct(0.1,1,self._accu_time_pre_round/1e3)
...
# It is done by using a container to store the tracking result of every ms and sum up them to 
# meet the requirement of accumulation time
# Nagetive results and positive results are stores in different container, this method is better 
# for signal with small noise, but worse for large noise.
class TrackPerMs_NPDevided(Tracking):
    def __init__(self,
                    sampRate:float,
                    sourcebuffer:Queue,
                    prn:int,
        ...
        super().__init__(sampRate,sourcebuffer,prn,codelen,initstate,res_buffer,ms_to_process,accu_time_ms)
        self.__track_loop_times=int(self.accu_time)               # The round of 1 ms loop needed to meet the requirement of accumulation time
        self.__track_results_P = np.zeros([self.__track_loop_times,6])   # The container stores the direct tracking results of early rounds, positive
        self.__track_results_N =np.zeros([self.__track_loop_times,6])   # The container stores the direct tracking results of early rounds, nagetive
        self._accu_time_pre_round=1         #1 ms
        # define the sychronizer to be used
        self.synchronizer=DirectSynchronizer(self.init_code_phase,self.init_code_frequency,self.init_carrier_phase,self.init_carrier_frequency,self.sample_rate,self.code_length,self.code)
        # define the discriminator to be used 
    ...
    def tracking_process(self):
        ...
        while(processed_time<=self.ms_to_process):
            ...
            # record direct tracking results and eliminate data bit
            # and update tracking results
            res_round=np.zeros(6,)
            res_round[:]=[Ip,Ie,Il,Qp,Qe,Ql]
            if(Ip<0):
                self.__track_results_N=np.roll(self.__track_results_N,1,0)
                self.__track_results_N[0]=res_round
            else:
                self.__track_results_P=np.roll(self.__track_results_P,1,0)
                self.__track_results_P[0]=res_round
           

            # calculate mean tracking results
            res_mean=np.zeros(6,)
            if Ip<0:
                res_mean=np.sum(self.__track_results_N,0)
            else:
                res_mean=np.sum(self.__track_results_P,0)
            ...
            # record final results of a ms and push to shared queue
            self.final_res[0:6]=[res_mean[0],res_mean[1],res_mean[2],res_mean[3],res_mean[4],res_mean[5]]
            self.final_res[6:10]=[cur_code_phase,cur_code_freq,cur_carrier_phase,cur_carrier_freq]
            self.final_res[10]=CN0_VSM
            self.final_res[13]=SNR
...
## ----------------------------------------------------
# created by mortarboard in 2022 06 05      
# A tracking method generate an output every ms
# No matter what the accumulation time is, this tracking method generates an output every ms. 
# It is done by using a container to store the tracking result of every ms and sum up them to 
# meet the requirement of accumulation time. 
# First, we calculate the mean value of hundreds of outputs and later subtrack the mean value to 
# make the output balanced in positive and negatives 
# This method revert the nagetive results to positives and later stores them in the container
# not finished yet!
class TrackPerMs_Balance(Tracking):
    def __init__(self,
                    sampRate:float,
                    sourcebuffer:Queue,
                    prn:int,
...
## ----------------------------------------------------
# created by mortarboard in 2022 06 07
# A tracking method generate an output every ms
# No matter what the accumulation time is, this tracking method generates an output every ms. 
# It is done by using a container to store the tracking result of every ms and sum up them to 
# meet the requirement of accumulation time. 
# This method won't process data bit shift
class TrackPerMs_NoBit(Tracking):
    def __init__(self,
                    sampRate:float,
                    sourcebuffer:Queue,
                    prn:int,
    ...
    def tracking_process(self):
        ...
        while(processed_time<=self.ms_to_process):
            ...
            # record direct tracking results and ignore data bit
            res_round=np.zeros(6,)
            res_round[:]=[Ip,Ie,Il,Qp,Qe,Ql]
...
# meet the requirement of accumulation time. 
# This method won't process data bit shift
# This method will add a filter to the output of integration
class TrackPerMs_NoBit_Filt(Tracking):
    def __init__(self,
                    sampRate:float,
                    sourcebuffer:Queue,
                    prn:int,
                    ...
                    res_buffer:Queue,               # A shared queue providing results of tracking for next procedure
                    ms_to_process=1e10,             #length to process
                    accu_time_ms=0                    #used as filter index
                    ) -> None:
        super().__init__(sampRate,sourcebuffer,prn,codelen,initstate,res_buffer,ms_to_process,20)
        self.__track_loop_times=int(self.accu_time)               # The round of 1 ms loop needed to meet the requirement of accumulation time
        self.__track_results = np.zeros([self.__track_loop_times,6])   # The container stores the direct tracking results of early rounds
        self._accu_time_pre_round=1         #1 ms
        # define the sychronizer to be used
        ...
        # define the loop filter of code phase to be used
        self.code_loopfilter=EulerOrder2Direct(0.1,1,self._accu_time_pre_round/1e3)
        self.filter_index=accu_time_ms
        pass
    ...
    def tracking_process(self):
        ...
        while(processed_time<=self.ms_to_process):
            ...
            # calculate mean tracking results
            filtered_res=(self.__track_results.T*(filtes[self.filter_index]+0.0001)).T
            res_mean=np.sum(filtered_res,0)*20
...
# This method won't process data bit shift
# This method will add a filter to the output of integration
class TrackPerMs_NoBit_FiltOrder(Tracking):
    def __init__(self,
                    sampRate:float,
                    sourcebuffer:Queue,
                    prn:int,
                    ...
                    res_buffer:Queue,               # A shared queue providing results of tracking for next procedure
                    ms_to_process=1e10,             #length to process
                    accu_time_ms=1                    #used as filter index
                    ) -> None:
        super().__init__(sampRate,sourcebuffer,prn,codelen,initstate,res_buffer,ms_to_process,accu_time_ms)
        self.__track_loop_times=int(self.accu_time)               # The round of 1 ms loop needed to meet the requirement of accumulation time
        self.__track_results = np.zeros([self.__track_loop_times,6])   # The container stores the direct tracking results of early rounds
    ...
    def tracking_process(self):
        ...
        while(processed_time<=self.ms_to_process):
            ...
            # calculate mean tracking results
            filtered_res=(self.__track_results.T*(filters_blackman_harris[self.accu_time]+0.0001)).T
            res_mean=np.sum(filtered_res,0)*20
